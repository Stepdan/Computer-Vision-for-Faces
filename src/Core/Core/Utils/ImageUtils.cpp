 ...
 */
cv::Mat QImage2cvMat(const QImage &img, int requiredMatType, MatColorOrder requriedOrder)
{
	...
	case 1:
		if (mat0.channels() == 3) {
			...
			if (srcOrder == MCO_BGRA)
				...
				cv::cvtColor(cvMat_argb2bgra(mat0), mat_adjustCn, CV_BGRA2GRAY);
		}
		break;
	case 3:
		if (mat0.channels() == 1) {
 ...
 */
QImage cvMat2QImage(const cv::Mat &mat, MatColorOrder order, QImage::Format formatHint)
{
 ...
 */
cv::Mat QImage2cvMat_shared(const QImage &img, MatColorOrder *order)
{
	if (img.isNull())
		return cv::Mat();
 ...
 */
QImage cvMat2QImage_shared(const cv::Mat &mat, QImage::Format formatHint)
{
...
cv::Mat QPixmap2cvMat(const QPixmap& pixmap, int requiredMatType/* = CV_8UC(0)*/, MatColorOrder requiredOrder /*= MCO_BGR*/)
{
	return QImage2cvMat(pixmap.toImage(), requiredMatType, requiredOrder);
}
